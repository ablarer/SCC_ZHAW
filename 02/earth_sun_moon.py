...
cm = np. array([7, 0, 1]) # start coordinates of moon # TODO Korrigieren
...
n_days = 36
n_moon = 2
n = n_days*n_per_day *n_moon                         # number of iterations
...
phi_spin_moon = 2*np.pi/n_days/n_per_day/n_moon      # 2 = positive spin of moon # TODO Korrigieren
...
for i in range(n):
    ...
    cm_rotated = am.rotate_around_point(cm, ce[0], ce[1], phi_rotate_earth)
    cm_tuple_rotated = am.rotate_tuple(cm_tuple_list[-1], math.sin(cm[0]), 100 * math.sin(phi_rotate_earth), phi_rotate_earth)
    cm_tuple_list.append(cm_tuple_rotated)
