...
def calculate_border_task1_solution(path_img, crop_factor, sigma):
    ...
    Remarks: Requires Installation of OpenCV (e.g. for Anaconda see https://www.geeksforgeeks.org/set-opencv-anaconda-environment/)
             enter the following in the Anaconda prompt: conda install -c menpo opencv
...
#    be converted with cv2.cvtColor(I1,cv2.COLOR_BGR2RGB)
    ...
    plt.subplot(331), plt.imshow(cv2.cvtColor(I1,cv2.COLOR_BGR2RGB)),plt.title('Original Image'),plt.axis('off')
...
# #    and show in a subplot(2,3,2)
    ...
    plt.subplot(332), plt.imshow(IG1,cmap='gray'), plt.title('Gray Scale Image'),plt.axis('off')
...
#    mask = np.empty_like(IG1, dtype=bool)
#    mask[:,:]=False
#    mask[xrange[0]:xrange[1],yrange[0]:yrange[1]] = True
#    cv2.drawContours(mask)

    plt.subplot(333), plt.imshow(IG2,cmap='gray'), plt.title('Cropped Image'),plt.axis('off')
...
# #     and show in a subplot(2,3,4)
    ...
    plt.subplot(234), plt.imshow(IG3,cmap='gray'), plt.title('Gaussian Filter'),plt.axis('off')
...
# #    see https://opencv24-python-tutorials.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_thresholding/py_thresholding.html
    ret,IG4 = cv2.threshold(IG3,0,1,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    plt.subplot(335), plt.imshow(IG4,cmap='gray'), plt.title('Otsu Binary Image'), plt.axis('off')   
...
# #    You will have to swap black and white in the binary picture, e.g. IG5 = -IG4+1
    ...
    plt.subplot(336), plt.imshow(IG7,cmap='gray'), plt.title('Filled Image'), plt.axis('off')    
...
# # 7) Compute the contours using cv2.findContours() with cv2.RETR_TREE and cv2.CHAIN_APPROX_NONE
    ...
    im = I2.astype(np.uint8)
    cv2.drawContours(im,contours,-1,(255,255,0),4)
    plt.subplot(337), plt.imshow(im,cmap='gray'), plt.title('Contour'), plt.axis('off')    
